...
#define USER_TX_PIN_NUMBER	25
#define USER_RX_PIN_NUMBER	24

#define MAX_FRAME_LENGTH 128
//#define	UART_DEBUG_PRINT

//#define countof(a)   sizeof(*(a))//(sizeof(a) / sizeof(*(a)))
//#define DEBUG_MODE_SIMULATE_UART_FLOW_IN	1
...
//	if((flagStartRx==true)&&(app_fifo_length(&gUartFifo)<=UART_FIFO_SIZE))
	...
	err_code=app_fifo_put(&upQueue,dataTemp);
	if(NRF_SUCCESS==err_code)
	{
		APP_ERROR_CHECK(err_code);
	}
...
//	packageDataTemp->length=sizeof(pHRD);//(heartRateDataPackageSample);
	...
	{
		uSystemTick10MsCnt=0;
		#ifdef DEBUG_MODE_SIMULATE_UART_FLOW_IN
